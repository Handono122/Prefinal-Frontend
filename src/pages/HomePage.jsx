...
class HomePage extends React.Component {
  ...
  onBtnAddFile = (e) => {
    if (e.target.files[0]) {
      ...
      reader.onload = () => {
        ...
        this.setState({ media: reader.result.split(',')[1] });
      };
  ...
  onUpload = () => {
    ...
    if (!this.state.addFile) {
      errors.addFile = 'File dibutuhkan';
    }
    ...
    const { addFile, caption, media, addFileName } = this.state;
    const formData = new FormData();
    formData.append('media', addFile);
    ...
    Axios.post('http://localhost:2001/media/upload', {
      ...
      .then((res) => {
        this.setState({ isLoading: false, isLogin: true, addFile: null, addFileName: '', caption: '', media: null });
        this.getPosts();
        let preview = document.getElementById('imgpreview');
        preview.src = '';
        let input = document.getElementById('imgpost');
        input.value = '';
        let captionPost = document.getElementById('caption');
        captionPost.value = '';
                ...
                <div className="col-md-12 grid-margin">
                  <div className="card rounded">
                    <textarea
                      ...
                      className={this.hasError('caption') ? 'form-control is-invalid' : 'form-control'}
                      id="caption"
                      rows="2"
                      placeholder="What are you doing?..."
                    ></textarea>
                    <div className="invalid-feedback">{this.state.errors.caption}</div>
                    ...
                    <input name="addFile" className={this.hasError('addFile') ? 'form-control is-invalid' : 'form-control'} type="file" id="imgpost" onChange={this.onBtnAddFile} />
                    <div className="invalid-feedback">{this.state.errors.addFile}</div>

                    <button className="btn btn-success pull-right mt-1" onClick={() => this.onUpload(this.state)}>
                      POST
                    </button>
                  </div>
                </div>
